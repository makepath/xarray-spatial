        ...
        for j in range(3):
            if i > 0:
                ...
                for k in range(chunk_locations[i][j]):
                    sum_previous += (array_locations[k][j][1] -
                                     array_locations[k][j][0])
...
def calc_chunksizes_rio(src_shape, src_coords,
                        chunks, numblocks, src_crs, dst_crs):
...
def _dask_reproject(arr, dst_crs, **kwargs):
    ...
    src_crs = arr.rio.crs
    chunks = calc_chunksizes_rio(arr.data.shape, src_coords,
                                 arr.data.chunks, arr.data.numblocks,
                                 src_crs, dst_crs)
...
def reprojection_rio(arr: xr.DataArray, dst_crs, **kwargs):
    ...
    reprojected = mapper(arr)(arr, dst_crs)
    return reprojected
