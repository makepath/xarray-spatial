...
Read `.codex/sweep-metadata-state.csv`.
...
5. After finishing (whether you found issues or not), update the inspection
   state file .codex/sweep-metadata-state.csv. Header:
   ...
   path = Path(".codex/sweep-metadata-state.csv")
   ...
   Then `git add .codex/sweep-metadata-state.csv` and commit it to the
...
```
column -t -s, .codex/sweep-metadata-state.csv | less
```
...
- If $ARGUMENTS is empty, use defaults: top 3, no category filter, no
  exclusions.
- State file (`.codex/sweep-metadata-state.csv`) is tracked in git, with
