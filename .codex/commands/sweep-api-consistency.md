...
Read `.codex/sweep-api-consistency-state.csv`.
...
5. Update .codex/sweep-api-consistency-state.csv using csv.DictReader/Writer:

   ```python
   import csv
   from pathlib import Path

   path = Path(".codex/sweep-api-consistency-state.csv")
...
- If $ARGUMENTS is empty, use defaults: top 3, no category filter, no
  exclusions.
- State file (`.codex/sweep-api-consistency-state.csv`) is tracked in
