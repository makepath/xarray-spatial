...
Read `.codex/sweep-style-state.csv`.
...
The file is covered by the `.codex/sweep-*-state.csv merge=union` rule in
...
5. After finishing (whether you found issues or not), update the inspection
   state file `.codex/sweep-style-state.csv`. The file is row-per-module
   CSV with header:
   ...
   path = Path(".codex/sweep-style-state.csv")
   ...
   Then `git add .codex/sweep-style-state.csv` and commit it to the
...
```
column -t -s, .codex/sweep-style-state.csv | less
```
...
- State file (`.codex/sweep-style-state.csv`) is tracked in git, covered by
  the `.codex/sweep-*-state.csv merge=union` rule in `.gitattributes` so
