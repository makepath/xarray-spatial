...
Read `.codex/sweep-security-state.csv`.
...
5. After finishing (whether you found issues or not), update the inspection
   state file .codex/sweep-security-state.csv. The file is row-per-module
   CSV with header:
   ...
   path = Path(".codex/sweep-security-state.csv")
   ...
   Then `git add .codex/sweep-security-state.csv` and commit it to the
...
```
column -t -s, .codex/sweep-security-state.csv | less
```
...
- State file (`.codex/sweep-security-state.csv`) is tracked in git, with
