...
Read `.codex/sweep-test-coverage-state.csv`.
...
5. Update .codex/sweep-test-coverage-state.csv:

   ```python
   import csv
   from pathlib import Path

   path = Path(".codex/sweep-test-coverage-state.csv")
...
- State file `.codex/sweep-test-coverage-state.csv` is tracked in git
  with `merge=union`.
