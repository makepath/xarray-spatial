...
def _place_same_crs(src_data, src_bounds, src_shape, y_desc,
    ...
    ``x_desc`` indicates that ``src_data`` is laid out with column 0 at
    the maximum x. The output is always emitted with ascending x, so the
    source window is reversed along its column axis before placement
    when ``x_desc`` is True.
    """
    out_h, out_w = out_shape
    src_h, src_w = src_shape
    ...
    if x_desc:
        # Source col 0 is at max x. The output column index `col_start_clip`
        # corresponds to source x = o_left + col_start_clip*o_res_x, which
        # is `src_w - 1 - (that col mapped from left side of source)` in
        # source column space. Equivalently, take the mirrored slice from
        # the source so that the leftmost output column receives the value
        # at the highest x in the source.
        src_c_start_mirrored = src_w - src_c_end
        src_c_end_mirrored = src_w - src_col_start
        src_window = np.asarray(
            src_data[src_row_start:src_r_end,
                     src_c_start_mirrored:src_c_end_mirrored],
            dtype=np.float64,
        )
