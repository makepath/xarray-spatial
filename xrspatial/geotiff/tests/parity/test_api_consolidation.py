...
The geotiff read/write surface is ``open_geotiff`` / ``to_geotiff`` plus
the ``build_vrt`` mosaic helper. The four data backends
(``_read_geotiff_dask``, ``_read_geotiff_gpu``, ``_read_vrt``,
``_write_geotiff_gpu``) are private; the dispatchers route to them from
the ``gpu=`` / ``chunks=`` / ``.vrt`` kwargs. ``build_vrt`` stays public
because it indexes files that already exist and has no DataArray to write.
...
def test_public_read_write_surface_is_consolidated():
    """The only lowercase ``__all__`` entries are the three public funcs."""
    fns = {name for name in g.__all__ if name[0].islower()}
    assert fns == {"open_geotiff", "to_geotiff", "build_vrt"}
...
def test_build_vrt_roundtrips_through_open_geotiff(tmp_path):
    """build_vrt is the public mosaic builder; open_geotiff reads it back."""
