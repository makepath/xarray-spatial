...
path only forwarded ``stable_only`` to ``_read_vrt`` for ``.vrt`` paths,
...
def test_dask_direct_fsspec_source_rejected_under_stable_only(memory_tiff_url):
    """The direct ``_read_geotiff_dask`` entry point gates remote sources too."""
    with pytest.raises(RemoteStableSourcesOnlyError) as excinfo:
        _read_geotiff_dask(memory_tiff_url, chunks=2, stable_only=True)
    _assert_remote_stable_error(excinfo)
...
#
# ``_read_geotiff_gpu`` is a public direct reader that routes HTTP / fsspec
# sources through the CPU fallback, so it must apply the same remote gate as
# ``open_geotiff`` and ``_read_geotiff_dask``. The gate runs before the cupy
...
def test_gpu_direct_fsspec_source_rejected_under_stable_only(memory_tiff_url):
    """The direct ``_read_geotiff_gpu`` entry point gates remote sources too."""
    with pytest.raises(RemoteStableSourcesOnlyError) as excinfo:
        _read_geotiff_gpu(memory_tiff_url, stable_only=True)
    _assert_remote_stable_error(excinfo)
...
def test_gpu_direct_http_source_rejected_under_stable_only():
    ...
    with pytest.raises(RemoteStableSourcesOnlyError) as excinfo:
        _read_geotiff_gpu("http://example.invalid/sample.tif", stable_only=True)
    _assert_remote_stable_error(excinfo)
