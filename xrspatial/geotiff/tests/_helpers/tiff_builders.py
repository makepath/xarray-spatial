"""In-memory TIFF builder used by the test suite.

Relocated from ``conftest.py`` so test modules can import the builder
directly without dragging in pytest fixture state. The function
signature and behaviour are unchanged; existing call sites continue to
work through the re-export in ``conftest.py``.
"""
