...
from xrspatial.geotiff._overview_kernels import KERNELS as _KERNELS

OVERVIEW_METHODS = ('mean', 'nearest', 'min', 'max', 'median', 'mode', 'cubic')

# Methods routed through the ngjit kernels in ``_overview_kernels`` when
# the input dtype is float. Integer dtypes still flow through the numpy
# nan-aware path below because the sentinel mask has to be computed at
# native integer width before any float promotion.
_NGJIT_REDUCE_METHODS = frozenset({'mean', 'min', 'max', 'median'})
...
def _block_reduce_2d(arr2d, method, nodata=None):
    ...
    existing fast path with no padding overhead.
    ...
    NaN in the output back to the sentinel before returning. Cubic on
    integer dtypes follows the same path via a float64 promotion so NaN
    can carry through the spline, with a
    ...
    # overview that no longer covers the source extent.
    oh, ow = (h + 1) // 2, (w + 1) // 2
    h2, w2 = 2 * oh, 2 * ow
    need_pad = (h2, w2) != (h, w)
    ...
    if method == 'cubic':
        ...
        # pixels back to the sentinel as expected. The
    ...
    if method == 'mode':
        ...
        # still holds. The result is cast back to the source dtype.
        if need_pad:
    ...
    # Fast path for float mean/min/max/median: route through the
    # type-specialized ngjit kernels in ``_overview_kernels``. The
    # kernel walks output pixels directly (no ``(oh, 2, ow, 2)``
    # reshape buffer), fuses the sentinel check into the read (no
    # intermediate ``np.where`` copy), and bounds-checks the right /
    # bottom edges so odd shapes do not pay for a NaN-pad copy. The
    # integer-dtype branch and the ``mode`` / ``cubic`` / ``nearest``
    # methods keep the numpy paths because the integer sentinel mask
    # must be computed at native dtype width.
    if arr2d.dtype.kind == 'f' and method in _NGJIT_REDUCE_METHODS:
        out = np.empty((oh, ow), dtype=arr2d.dtype)
        if nodata is not None and not np.isnan(nodata):
            try:
                sentinel_val = arr2d.dtype.type(nodata)
                has_sentinel = True
            except (OverflowError, ValueError):
                sentinel_val = arr2d.dtype.type(0)
                has_sentinel = False
        else:
            sentinel_val = arr2d.dtype.type(0)
            has_sentinel = False
        _KERNELS[method](arr2d, out, sentinel_val, has_sentinel)
        return out
    ...
    # nan-aware aggregations below. The padded buffer
    ...
    if arr2d.dtype.kind == 'f':
        ...
        # and poisons the overview. Match the upstream
...
def _make_overview(arr: np.ndarray, method: str = 'mean',
    ...
    nodata : scalar or None
        ...
        result. Applies to both float dtypes (including ``cubic``) and
        integer dtypes (the mean / min / max /
