...
from xrspatial.hydro.watershed_dinf import watershed_dinf
...
class TestMemoryGuard:
    ...
    def test_numpy_huge_raster_raises(self):
        ...
        pour_points[0, 0] = 1.0
        fd_da, pp_da = _make_fd_and_pp(flow_dir, pour_points, backend='numpy')

        with patch(
            "xrspatial.hydro.watershed_dinf._available_memory_bytes",
            return_value=1,
        ):
            with pytest.raises(MemoryError, match="working memory"):
                watershed_dinf(fd_da, pp_da)
    ...
    def test_dask_path_skips_guard(self):
        ...
        pour_points[0, 0] = 1.0
        fd_da, pp_da = _make_fd_and_pp(
            flow_dir, pour_points, backend='dask', chunks=(3, 3))
    ...
    def test_cupy_huge_raster_raises(self):
        ...
        pour_points = np.full((4, 4), np.nan, dtype=np.float64)
        pour_points[0, 0] = 1.0
        fd_da, pp_da = _make_fd_and_pp(flow_dir, pour_points, backend='cupy')

        with patch(
            "xrspatial.hydro.watershed_dinf._available_gpu_memory_bytes",
            return_value=1,
        ):
