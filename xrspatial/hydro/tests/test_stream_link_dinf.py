...
class TestMemoryGuard:
    ...
    def test_numpy_huge_raster_raises(self):
        ...
        with patch(
            "xrspatial.hydro.stream_link_dinf._available_memory_bytes",
            return_value=1,
        ):
    ...
    def test_dask_path_skips_guard(self):
        ...
        da_a = create_test_raster(angles, backend='dask', chunks=(3, 3))
        da_fa = create_test_raster(accum, backend='dask', chunks=(3, 3))

        with patch(
            "xrspatial.hydro.stream_link_dinf._available_memory_bytes",
            return_value=1,
        ):
            result = stream_link_dinf(da_a, da_fa, threshold=1)
    ...
    def test_cupy_huge_raster_raises(self):
        ...
        with patch(
            "xrspatial.hydro.stream_link_dinf._available_gpu_memory_bytes",
            return_value=1,
        ):
            with pytest.raises(MemoryError, match="GPU working memory"):
                stream_link_dinf(cp_a, cp_fa, threshold=1)
